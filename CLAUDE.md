...
- 지점 구조 (지점 키 = config.js BRANCHES): **`용산 1호점`, `서초 2호점`, `피티앤골프 3호점`** (3호점은 물리적으로 용산 소재). **용산·서초는 PT 단독만 발송, 피티앤골프 3호점은 PT 단독·골프 단독·PT+골프 통합 모두 발송** (config.js `CONTRACT_TYPES_BY_BRANCH` → admin 약관종류 드롭다운 지점별 분기). (구 지점 키 `용산점/서초점/골프스튜디오` 는 2026-06-22 이름 변경; 그 이전 서명 계약의 contracts.branch 에는 구 키가 박제됨)
...
- **약관 조항 추가 세대 `2026-09-21`** (서초 `2026-09-21-seocho`) — **대표 SQL 재실행 1회 필요**. 스키마의 insert…select 가 구버전(2026-06-22/2026-06-10) 본문을 읽어 조항을 1회만 붙여 새 버전 생성(idempotent), 구버전 + 중간세대 2026-09-17 은 비활성화. 기서명 계약은 template_id + 서명시점 스냅샷으로 고정되어 무영향. **별도 동의 체크박스는 두지 않음** (약관 전문 100% 스크롤 + '약관 전문 동의'로 커버, 불필요 항목은 회원 반감 — 직원 피드백 이력)
  - **§5 분납(분할 납입) — 전 약관 4종**: 총액·전체회차 기준 계약 성립 / 미납 시 잔여 회차 보류 / 환급은 §3 환불 규정
  - **§6 패밀리세션(2인 합산 50회) — PT 약관만**(combo·golf 제외, PT 회차 상품 전용): 각자 최소 10회 이상 이용 조건의 할인 / 본인만 수강 / 한 명 30회 도달 시 상대가 10회 미달이면 30회 초과분 보류 / 조건 미충족 종료 시 **이용 회차를 개인 30회권 회당 단가로 계산하고 잔액 현금 환불**(추가 청구·추가 회차 없음) / 회차 이전은 양측 10회 이상 후. 이 정산식은 대표가 직접 방향 잡음(센터 무손실 + 계산 단순 + 회원 기분 상하지 않게) — 30회권 단가가 패밀리 단가보다 높아 손실이 없고, 회원은 개인 정가 이하만 부담. 근거·수치 `docs/PT-패밀리세션-분납-계약검토.md`
- **분납 발송 UI (admin)**: **☑ 분납 계약(2회)** 체크 → 1·2차 금액(50/50 자동 제안)·일자(1차=오늘, 2차=1차+1개월 자동). **총 결제금액이 바뀌는 모든 경로**(상품 선택·직접 입력 항목 금액, 총액 직접 입력)에서 `syncInstallment()` 가 재계산하고 **비고란에 실시간 기재**(+미리보기). 1차를 고치면 2차가 자동으로 맞춰져 합계는 항상 총액. 무효면 비고에 안 쓰고 경고 표시. 해제/무효 시 `[분납]` 줄이 있을 때만 비고를 건드려 직원 비고·줄바꿈 보존. 카톡 발송문에 분납 일정 1줄. 원칙: 첫 결제 시 총액 기준 계약서 1건, 2회까지(3회↑=할부거래법 → 카드할부 안내), 비고에 별도 정산식 금지(§5가 규정). 순수 함수는 admin.js `installment-pure-start/end` 마커 사이(node 테스트용, DOM 스텁 12 시나리오 통과). 기준 문서 `docs/분납-계약-기준.md`. (매니저 검토문서의 **환불조항 개정은 대표 지시로 미채택** — 현행 §3 유지)

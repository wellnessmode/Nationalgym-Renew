...
- **분납(2회) 계약 기준 (2026-09-17)**: 전 약관 4종(PT공통·PT서초·combo·golf) 신규 버전 `2026-09-17`(서초 `2026-09-17-seocho`)에 **"5. 분납(분할 납입)" 상시 조항** 추가(총액 기준 계약 성립 · 미납 시 잔여 회차 보류 · 환급은 §3 환불 규정). 스키마의 insert…select 블록이 구버전 본문에 §5 를 1회 붙여 새 버전 생성(idempotent), 구버전은 비활성화 → **대표 SQL 재실행 1회 필요**. admin.html **☑ 분납 계약(2회)** 체크 → 1차/2차 금액(50/50 자동 제안)·일자 → 합계≠총액·2차일≤1차일이면 발송 차단 → 비고란 맨 앞에 `[분납] 총 결제금액 …을 2회 분납 — 1차 …(납입) / 2차 …(납입 예정). 분납 관련 사항은 약관 제5조에 따름.` 자동 기재(재생성 시 이전 [분납] 줄 제거) + 카톡 발송문에 분납 일정 1줄. 원칙: 첫 결제 시 총액 기준 계약서 1건, 분납은 2회까지(3회↑=할부거래법 → 카드할부 안내), 비고에 별도 정산식 금지. 순수 함수는 admin.js `installment-pure-start/end` 마커 사이(node 테스트용). 기준 문서 `docs/분납-계약-기준.md`. (매니저 검토문서의 환불조항 개정·패밀리세션 특약은 대표 지시로 미채택)

...
-- ===========================================================================
-- 조항 추가 세대 2026-09-21 — §5 분납(전 약관) + §6 패밀리세션(PT 약관만)
--   ① §5 분납: 활성 약관 4종(PT 공통·PT 서초·combo·golf) 전부에 상시 조항으로 추가.
--   ② §6 패밀리세션(2인 합산 50회): PT 약관(공통·서초)에만 추가. combo·golf 는 미적용
--      (패밀리세션은 PT 회차 상품 전용. 3호점의 PT 단독 발송은 PT 약관을 쓰므로 커버됨).
--   구버전 행(2026-06-22 / 2026-06-10)의 본문을 읽어 조항을 1회만 붙이므로 재실행해도
--   이중 삽입 없음(idempotent). 기서명 계약은 template_id + 서명시점 스냅샷으로 고정되어 무영향.
--   별도 동의 체크박스는 두지 않음 — 약관 전문 100% 스크롤 + "약관 전문 동의" 로 커버되고,
--   패밀리 아닌 대다수 PT 회원에게 불필요한 항목이 노출되어 반감만 키움(직원 피드백 이력).
--   분납 조건(회차·금액·예정일)은 admin 의 '분납 계약(2회)' 체크로 비고란에 자동 기재됨.
-- ===========================================================================
insert into public.contract_templates (contract_type, version, branch, title, body_html, agreements_json, privacy_json, refund_policy_json)
...
<li>분납 여부와 관계없이 중도 해지 시의 환급은 본 계약서의 환불 규정(제3조)에 따릅니다.</li>
    ...
    || case when t.contract_type = 'pt' then $s6$
<h3>6. 패밀리세션(2인 합산 50회) 특약</h3>
<ul>
<li>패밀리세션 단가는 <b>두 분이 각자 본인 명의로 최소 10회 이상 이용</b>하는 것을 조건으로 적용되는 할인 단가이며, 개인 단독 등록에는 적용되지 않습니다.</li>
<li>수업은 <b>계약 명의자 본인만</b> 이용할 수 있습니다. 타인이 대신 수강한 회차는 해당 이용자가 진행한 회차로 인정되지 않습니다.</li>
<li>한 분의 진행 회차가 30회에 이른 시점에 다른 한 분의 진행 회차가 10회에 미치지 못하는 경우, 센터는 다른 한 분이 10회를 진행할 때까지 <b>30회를 초과하는 회차의 진행을 보류</b>합니다.</li>
<li>위 조건이 끝내 충족되지 않는 경우(한 분이 이용 의사 없음을 통지하거나 유효기간이 만료된 경우), 각 이용자가 <b>실제 이용한 회차를 센터의 개인 등록 30회권 기준 회당 단가로 계산</b>하고, 남은 금액은 현금으로 환불합니다. 이 경우 회원에게 추가 금액을 청구하지 않으며, 회차를 추가로 부여하지도 않습니다.</li>
<li>이용자 간 잔여 회차의 이전은 <b>두 분 모두 10회 이상 진행한 이후</b>에만 가능합니다.</li>
</ul>$s6$ else '' end,
       t.agreements_json, t.privacy_json, t.refund_policy_json
  from (values
          ('pt',    '2026-06-22',        '2026-09-21'),
          ('pt',    '2026-06-22-seocho', '2026-09-21-seocho'),
          ('combo', '2026-06-10',        '2026-09-21'),
          ('golf',  '2026-06-10',        '2026-09-21')
...
--   현행(2026-09-21): PT 공통 2026-09-21(§5+§6) / PT 서초 2026-09-21-seocho(§5+§6) /
--   combo 2026-09-21(§5) / golf 2026-09-21(§5).
--   2026-09-17 세대(§5 만 있던 중간 세대)도 함께 비활성 — 먼저 실행했더라도 안전하게 흡수.
update public.contract_templates set is_active = false
 where (contract_type, version) in (
   ...
   ('pt','2026-09-17'), ('pt','2026-09-17-seocho'), ('combo','2026-09-17'), ('golf','2026-09-17')
 );
